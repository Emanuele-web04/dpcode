...
const LOOKBACK_MS = LOOKBACK_DAYS * ONE_DAY_MS;
const USAGE_CACHE_TTL_MS = 30_000;
const MAX_RECENT_USAGE_FILES = 200;
...
function buildUsageLines(input: {
  ...
  return [
    ...
    {
      label: "Latest",
      value: formatUsageTimestamp(input.latestTimestampMs),
      ...(input.latestSubtitle ? { subtitle: input.latestSubtitle } : {}),
    },
  ];
}
...
async function loadCodexUsageSnapshot(input: {
  ...
  const cutoff7d = nowMs - LOOKBACK_MS;
  ...
  return {
    ...
    usageLines: buildUsageLines({
      ...
      latestTimestampMs: latestSummary.timestampMs,
      latestSubtitle: "local Codex archive",
    }),
...
async function loadClaudeUsageSnapshot(input: { homeDir: string }): Promise<UsageSnapshot | null> {
  ...
  return {
    ...
    usageLines: buildUsageLines({
      ...
      latestTimestampMs: latestSample.timestampMs,
      latestSubtitle: latestSample.model ?? "local Claude transcripts",
    }),
